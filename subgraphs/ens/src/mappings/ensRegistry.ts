...
function _handleNewOwner(event: NewOwnerEvent, isMigrated: boolean): void {
	const account = new Account(event.params.owner.toHexString());
	account.save();
	...
	const domainEvent = new NewOwner(createEventID(event));
...
export function handleTransfer(event: TransferEvent): void {
	...
	const domainEvent = new Transfer(createEventID(event));
	domainEvent.blockNumber = event.block.number.toI32();
	domainEvent.transactionID = event.transaction.hash;
...
export function handleNewResolver(event: NewResolverEvent): void {
	const id = event.params.resolver.toHexString().concat('-').concat(event.params.node.toHexString());
	...
	const domainEvent = new NewResolver(createEventID(event));
	domainEvent.blockNumber = event.block.number.toI32();
	domainEvent.transactionID = event.transaction.hash;
	domainEvent.domain = node;
...
export function handleNewTTL(event: NewTTLEvent): void {
	...
	const domainEvent = new NewTTL(createEventID(event));
	domainEvent.blockNumber = event.block.number.toI32();
	domainEvent.transactionID = event.transaction.hash;
	domainEvent.domain = node;
