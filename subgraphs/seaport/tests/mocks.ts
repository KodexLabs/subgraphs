...
export function createOrderValidatedEvent(validator: Address, orderHash: Bytes, offerer: Address, zone: Address): OrderValidatedEvent {
	...
	const offererParam = new ethereum.EventParam('offerer', ethereum.Value.fromAddress(offerer));
	const zoneParam = new ethereum.EventParam('zone', ethereum.Value.fromAddress(zone));
