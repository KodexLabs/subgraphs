...
program
	...
	.action((subgraph, network) => {
		...
		exec(
			`node_modules/.bin/graph deploy --product hosted-service kodexlabs/${subgraph}-${network} subgraphs/${subgraph}/subgraph.yaml`
		).stdout.pipe(process.stdout);
	});
...
program
	...
	.action((subgraphName = 'kodexlabs/momiji', options) => {
		const method = {
